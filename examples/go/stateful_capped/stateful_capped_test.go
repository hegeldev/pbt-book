...
// The same state machine as stateful/stateful_test.go, but driving CappedCache
// (which stops accepting new keys once it is full) instead of MyLRUCache.
type cacheModel struct {
	cache    *CappedCache[string, int]
	capacity int
...
func TestCacheMatchesModel(t *testing.T) {
	hegel.Test(t, func(ht *hegel.T) {
		...
		m := &cacheModel{
			cache:    NewCappedCache[string, int](capacity),
			capacity: capacity,
